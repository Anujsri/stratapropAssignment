...
def send_mail(name,email,device_name,device_id,model_name,message):
    ...
    try:
        ...
        msg.html = 'Hi ' + name + ',<br>' +  message + '<br><div style="margin-right:-15px;margin-left:-15px;">' + '<div style="position:relative;min-height:1px;padding-right:15px;padding-left:15px;@media (min-width:992px){float:left};width:66.66666667%;margin-left:16.66666667%;margin-right:16.66666667%;" >' +'<div  style="padding-left:0;margin-bottom:20px">' +'<li style="position:relative;display:block;padding:10px 15px;margin-bottom:-1px;background-color:#5499C7;color : white;font-size: 20px;"><center><b>Device Details</b></center></li>' +'<li style="position:relative;display:block;padding:10px 15px;margin-bottom:-1px;background-color:#fff;border:1px solid #AEB6BF"><b>Name : </b>' + device_name + '</li>' +'<li style="position:relative;display:block;padding:10px 15px;margin-bottom:-1px;background-color:#fff;border:1px solid #AEB6BF"><b>Device Id. :  </b>'  + device_id + '</li>' +'<li style="position:relative;display:block;padding:10px 15px;margin-bottom:-1px;background-color:#fff;border:1px solid #AEB6BF"><b>Model. :  </b>' + model_name + '</li>' +'</div>' +'</div>' +'</div>'
        mail.send(msg)
    except:
        import traceback
